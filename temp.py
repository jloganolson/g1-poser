...
if __name__ == "__main__":
    ...
    class GlobalGaitControls(ttk.LabelFrame):
        def __init__(self, parent: tk.Misc) -> None:
            ...
            # Refinement controls
            self.stance_linear = tk.BooleanVar(value=True)   # constant horizontal speed in stance                                                                                   
            self.coplanar = tk.BooleanVar(value=True)        # clamp stance z to plane
            self.plane_hands = tk.BooleanVar(value=False)    # include hands in coplanar constraint                                                                                  
            self.plane_z = tk.DoubleVar(value=0.0)
            self.sync_speed = tk.BooleanVar(value=False)     # enforce consistent linear speed across all feet
            ...
            # Refinement UI rows - test one by one
            ttk.Checkbutton(self, text="Linear stance speed", variable=self.stance_linear).grid(row=6, column=0, sticky="w")                                                         
            ttk.Checkbutton(self, text="Coplanar stance (ground plane)", variable=self.coplanar).grid(row=7, column=0, sticky="w")                                                   
            ttk.Checkbutton(self, text="Apply plane to hands (front)", variable=self.plane_hands).grid(row=8, column=0, sticky="w")
            ttk.Checkbutton(self, text="Synchronized foot speed", variable=self.sync_speed).grid(row=9, column=0, sticky="w")                                                  
            ttk.Label(self, text="Plane z (m)").grid(row=10, column=0, sticky="w")
            # Use Label instead of Entry to avoid threading issues
            self._plane_display = ttk.Label(self, text="0.000", width=10, relief="sunken", anchor="e")
            self._plane_display.grid(row=10, column=1, sticky="w", padx=(6, 0))
            # Placeholder; will be wired after model/data are available
            self.fit_btn = ttk.Button(self, text="Fit plane now", command=lambda: None)
            self.fit_btn.grid(row=10, column=2, sticky="w", padx=(8, 0))
            
            # Wire up plane_z variable to update display
            def _update_plane_display(*_args):
                try:
                    self._plane_display.configure(text=f"{float(self.plane_z.get()):.3f}")
                except Exception:
                    pass
            self.plane_z.trace_add("write", _update_plane_display)
        ...
        def reset(self) -> None:
            ...
            # Refinement defaults
            self.stance_linear.set(True)
            self.coplanar.set(True)
            self.plane_hands.set(False)
            self.sync_speed.set(False)
        ...
        def snapshot(self) -> dict:
            return {
                ...
                "linear_stance": bool(self.stance_linear.get()),
                "coplanar": bool(self.coplanar.get()),
                "plane_hands": bool(self.plane_hands.get()),
                "plane_z": float(self.plane_z.get()),
                "sync_speed": bool(self.sync_speed.get()),
            }
        
        def set_plane_z(self, plane: float) -> None:
            self.plane_z.set(float(plane))
            # Update the display label
            try:
                self._plane_display.configure(text=f"{float(plane):.3f}")
            except Exception:
                pass
        
        def get_plane_z(self) -> float:
            return float(self.plane_z.get())
    ...
    # Wire up "Fit plane now" to measure current stance site heights
    # Initialize with defaults, will be populated after viewer starts
    site_ids = {"FL": -1, "FR": -1, "RL": -1, "RR": -1}

    def _leg_included_in_plane(leg_key: str) -> bool:
        if bool(global_ctrl.plane_hands.get()):
            return True
        # Only rear legs (feet)
        return leg_key in ("RL", "RR")

    def _fit_plane_now() -> None:
        try:
            # Compute phase per leg to filter by stance
            ...
            for leg in ("FL", "RR", "FR", "RL"):
                if not _leg_included_in_plane(leg):
                    continue
                sid = site_ids.get(leg, -1)
                if sid < 0:
                    continue
            ...
            # Fallback: if no stance legs captured, use all included legs
            if not zs:
                for leg in ("FL", "RR", "FR", "RL"):
                    if not _leg_included_in_plane(leg):
                        continue
            ...
            if zs:
                plane = min(zs)
                global_ctrl.set_plane_z(plane)
        except Exception:
            pass

    try:
        global_ctrl.fit_btn.configure(command=_fit_plane_now)
    except Exception:
        pass
    ...
    def _reset_all() -> None:
        ...
        try:
            ...
            # Add Reset button to global controls (placed after refinement rows)
    ttk.Button(global_ctrl, text="Reset", command=_reset_all).grid(row=11, column=0, sticky="w", pady=(6, 0))
    ...
    def _apply_config(cfg: dict) -> None:
        ...
        try:
            ...
            try:
                ...
                if "linear_stance" in g:
                    global_ctrl.stance_linear.set(bool(g.get("linear_stance", bool(global_ctrl.stance_linear.get()))))                                                               
                if "coplanar" in g:
                    global_ctrl.coplanar.set(bool(g.get("coplanar", bool(global_ctrl.coplanar.get()))))                                                                              
                if "plane_hands" in g:
                    global_ctrl.plane_hands.set(bool(g.get("plane_hands", bool(global_ctrl.plane_hands.get()))))                                                                     
                if "plane_z" in g:
                    try:
                        global_ctrl.plane_z.set(float(g.get("plane_z", float(global_ctrl.plane_z.get()))))                                                                           
                    except Exception:
                        pass
                if "sync_speed" in g:
                    global_ctrl.sync_speed.set(bool(g.get("sync_speed", bool(global_ctrl.sync_speed.get()))))
            except Exception:
                pass
    ...
    ttk.Button(global_ctrl, text="Import…", command=_on_import).grid(row=11, column=1, sticky="w", pady=(6, 0), padx=(8, 0))         
    ttk.Button(global_ctrl, text="Export", command=_on_export).grid(row=11, column=2, sticky="w", pady=(6, 0), padx=(8, 0))
    ...
    def _on_export_animation() -> None:
        try:
            ...
            # Settings
            linear_stance = bool(global_ctrl.stance_linear.get())
            coplanar = bool(global_ctrl.coplanar.get())
            include_hands = bool(global_ctrl.plane_hands.get())

            def _leg_included(leg: str) -> bool:
                return True if include_hands else (leg in ("RL", "RR"))

            # Optional pre-pass to estimate plane z from stance site heights
            plane_z = float(global_ctrl.get_plane_z())
            if coplanar:
                # Reset offline state
                offline_cfg.data.qpos[:] = configuration.data.qpos[:]
                mujoco.mj_forward(offline_cfg.model, offline_cfg.data)
                z_samples: list[float] = []
                # Temporary rollout only to gather stance heights
                for k in range(total_steps):
                    ...
                    for leg in ("FL", "RR", "FR", "RL"):
                        ...
                        if in_stance:
                            ...
                            # use linear interpolation for target sampling
                            tx = (1.0 - ss) * lift_x + ss * plant_x
                            ty = (1.0 - ss) * lift_y + ss * plant_y
                            tz = base_z
                        else:
                    ...
                    # Collect stance z from resulting site positions
                    for leg in ("FL", "RR", "FR", "RL"):
                        if not _leg_included(leg):
                            continue
                        phi_total = t_local / T + float(phase_offsets[leg])
                        phi = phi_total - math.floor(phi_total)
                        if phi >= duty:
                        ...
                        # Map to site index
                        if leg == "FL":
                            sid = mujoco.mj_name2id(offline_cfg.model, mujoco.mjtObj.mjOBJ_SITE, "left_palm")
                        elif leg == "FR":
                            sid = mujoco.mj_name2id(offline_cfg.model, mujoco.mjtObj.mjOBJ_SITE, "right_palm")
                        elif leg == "RL":
                            sid = mujoco.mj_name2id(offline_cfg.model, mujoco.mjtObj.mjOBJ_SITE, "left_foot")
                        else:
                            sid = mujoco.mj_name2id(offline_cfg.model, mujoco.mjtObj.mjOBJ_SITE, "right_foot")
                        if sid >= 0:
                            z_samples.append(float(offline_cfg.data.site_xpos[sid][2]))

                if z_samples:
                    plane_z = min(z_samples)

                # Reset state before the real export rollout
                offline_cfg.data.qpos[:] = configuration.data.qpos[:]
                mujoco.mj_forward(offline_cfg.model, offline_cfg.data)

            # Offline rollout for three cycles (continuous integration across cycles) with constraints
            for k in range(total_steps):
                t_local = float(k) * dt
                for leg in ("FL", "RR", "FR", "RL"):
                    s = ui_snap[leg]
                    ...
                    if in_stance:
                        ...
                        if coplanar and _leg_included(leg):
                            tz = float(plane_z)
                        else:
                            tz = base_z
                    else:
    ...
    ttk.Button(global_ctrl, text="Export Animation", command=_on_export_animation).grid(row=12, column=0, sticky="w", pady=(6, 0))
    ...
    with mujoco.viewer.launch_passive(model=model, data=data, show_left_ui=False, show_right_ui=False) as viewer:
        ...
        # Now that viewer is started, safely populate site IDs
        try:
            site_ids["FL"] = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_SITE, "left_palm")
            site_ids["FR"] = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_SITE, "right_palm")
            site_ids["RL"] = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_SITE, "left_foot")
            site_ids["RR"] = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_SITE, "right_foot")
        except Exception:
            print("Warning: Could not resolve site IDs")
        ...
        while viewer.is_running():
            ...
            for leg in LEG_ORDER:
                ...
                if phi < duty:
                    # Stance: optionally linear speed and optional coplanar z
                    s = phi / max(1e-6, duty)
                    if bool(global_ctrl.stance_linear.get()):
                        tx = (1.0 - s) * lift_x + s * plant_x
                        ty = (1.0 - s) * lift_y + s * plant_y
                    else:
                        s_smooth = _smoothstep(s)
                        tx = (1.0 - s_smooth) * lift_x + s_smooth * plant_x
                        ty = (1.0 - s_smooth) * lift_y + s_smooth * plant_y
                    if bool(global_ctrl.coplanar.get()) and (_leg_included_in_plane(leg) or bool(global_ctrl.plane_hands.get())):
                        tz = float(global_ctrl.get_plane_z())
                    else:
                        tz = base_z
                else:
